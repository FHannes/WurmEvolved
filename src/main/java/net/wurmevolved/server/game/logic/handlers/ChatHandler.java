...
public class ChatHandler extends LogicHandler implements CommandCaller {
    ...
    @Override
    public void join() {
        sendLocal(String.format("%s has joined the game!", player.getUsername()));
    }

    @Override
    public void handle(AbstractPacket packet) {
        if (packet instanceof ClientMessagePacket) {
            String msg = ((ClientMessagePacket) packet).getMessage();
            if (msg.equals("")) {
                return;
            }
            if (msg.startsWith("/")) {
                ...
                msg = parseMsg(msg);
                switch (((ClientMessagePacket) packet).getChannel()) {
                    case ":Local":
    ...
    @Override
    public void leave() {
        sendLocal(String.format("%s has left the world!", player.getUsername()));
    }
